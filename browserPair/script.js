...
//var uiTime = $("#time");
var uiClick = $("#click");//click amount
var uiTimer = $("#timer");
var uiPlay = $("#gamePlay");
...
$(function(){
	...
	loader.addCompletionListener(function() {
		uiPlay.text('PlayAAAA');
		ui.addClass('open');
	});
	loader.start();//delegate to PxLoaderImage.start()
	init();
});
...
function init() {
	...
	//Handle the melody
	/**Note: not getting the event target, but directly use DOM to get the element**/
...
//click one card and mark as flipped
function selectCard() {
...
function checkPattern() {
	...
	if (pattern) {
		...
		if(!decent) { //workaround for not support transition browsers
			removeTookCards();
		}
	} else {
...
function isMatchPattern() {
	...
	var pattern = $(cards[0]).data("pattern");//HTML5 data-* attribute
...
function reStartGame(){
	...
	clearTimeout(scoreTimeout);
	matchingGame.deck = $.extend(true, [], matchingGame.clone);//what does it mean?
	startGame();
}
